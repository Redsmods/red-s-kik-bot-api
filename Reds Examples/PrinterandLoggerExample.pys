...
# def on_login_ended(self, response: LoginResponse):
       # print("Full name: {} {}".format(response.first_name, response.last_name))
       #it has to be set up a certain way for the prints to work.
       #but say you wanted to add the JID to display
       #you would do this
       
# def on_login_ended(self, response: LoginResponse):
       # print("Full name: {} {} JID: {}".format(response.first_name, response.last_name, from_jid))
       # i dont know if this works, its been a while but here if that example is wrong remove "from_jid" and add "chat_message.from_jid"
       
       def 
       
       
       
       #examples below from tomer API 
...
#this isnt all the prints in Tomer API
#study the Tomer API Code
